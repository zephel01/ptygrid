...
# Phase 4.x 準備契約（teams pane backend: socket RPC — スタンドアロン実装・未配線）

Claude Code Agent Teams / subagent のペイン自動追加
（[docs/spec-claude-teams-panes.md](docs/spec-claude-teams-panes.md)）に向けた基盤。
`src-tauri/teams-backend/` に **appクレートから独立したスタンドアロンcrate** として実装し、
本sectionの時点では `ptygrid` 本体のIPC・MCP・UIへ**一切配線していない**（wire format、
Tauri commands/events、Queen toolsに変更なし）。Phase 4.2でapp側が`PaneHost`を実装して
配線する際、本契約を出発点とする。

## プロトコル概要

CustomPaneBackend提案（anthropics/claude-code#26572 draft、protocol_version `"1"`）に
準拠したJSON-RPC 2.0。transportはUnix domain socket、framingはNDJSON（1 object = 1行）。
提案が上流採用された場合、同一socketを`CLAUDE_PANE_BACKEND_SOCKET`としてそのまま
広告できる形を保つ。入力の`jsonrpc`メンバーは省略可（#26572の例に合わせる）、出力は
常に`"jsonrpc":"2.0"`を付す。

## Handshake

すべてのconnectionは最初に`initialize`を成功させる必要がある。それ以前の他requestは
`-32002`で拒否する。

| method | params | result |
|---|---|---|
| `initialize` | `{ protocol_version: "1", capabilities: string[], auth_token?: string }` | `{ protocol_version: "1", capabilities: ["events","capture"], self_context_id }` |

- `auth_token`は**ptygrid拡張**（#26572には無い）。serverにtoken設定がある場合は一致必須、
  不一致・欠落は`-32002`を返してconnectionをcloseする。tokenless serverは無視する。
- `protocol_version`不一致は`-32602`。
- `initialize`成功後、そのconnectionへ`context_exited` push eventの転送を開始する。
- 2回目の`initialize`は`-32600`。

## Methods（initialize後）

| method | params | result | 説明 |
|---|---|---|---|
| `spawn_agent` | `{ command: string[], cwd?, env?: object, metadata?: {name?,color?,role?} }` | `{ context_id }` | teammate process起動。`command`はargv（shell文字列不可）。allowlist違反は`-32001` |
| `write` | `{ context_id, data }` | `{}` | `data`はbase64。復号後bytesをstdinへ |
| `capture` | `{ context_id, lines? }` | `{ text }` | scrollback読取（`lines`省略時は全量） |
| `kill` | `{ context_id }` | `{}` | context終了。autorestartを発火させない |
| `list` | `{}` | `{ contexts: string[] }` | self含む生存context ID列挙 |
| `get_self_id` | `{}` | `{ context_id }` | 呼出元clientの動作context |
| `ptygrid/focus` | `{ context_id }` | `{}` | **ptygrid拡張**: pane focus（tmux `select-pane`対応） |

## Push events（server → client、id無しnotification）

| method | params |
|---|---|
| `context_exited` | `{ context_id, exit_code: number\|null }` |

## Error codes

標準JSON-RPC（`-32700` parse / `-32600` invalid request / `-32601` method not found /
`-32602` invalid params / `-32603` internal）に加え、実装定義:

| code | 意味 |
|---|---|
| `-32000` | context not found |
| `-32001` | spawn denied（teammate binary allowlistなど） |
| `-32002` | not authenticated（initialize未実施 / token不一致） |

## Socket / 環境変数

- socket: app-data配下 `teams/run/` を想定（dir 0700 / socket 0600）。`bind_socket()`が
  stale socketの除去とpermission設定を行う。
- `PTYGRID_TEAMS_SOCK`: shimが接続するsocket path（host lead起動時にappが注入）。
- `PTYGRID_TEAMS_TOKEN`: 任意のper-lead token。設定時はshimが`initialize.auth_token`で送る。
- context IDはopaque文字列だが、tmux互換のため`%<n>`形式を推奨する（shimはIDを
  そのままtmux pane IDとして出力する）。`MockPaneHost`とapp実装はこの形式に従う。

## `ptygrid-tmux-shim`（同crateのbinary）

tmux互換シム。1-shot: 1 tmux subcommand = 1 connection（`initialize`→request→exit）。
tmux subcommandのRPC対応:

| tmux | RPC | 備考 |
|---|---|---|
| `split-window [-d -h -v -P -F fmt -c cwd] [--] cmd` | `spawn_agent` | 単一shell文字列は簡易word splitでargv化（quote対応・展開なし）。`-P`でcontext_idをstdoutへ |
| `send-keys [-l] -t <id> keys...` | `write` | key名変換（Enter/Tab/Escape/Space/BSpace/矢印/C-x）。`-l`はliteral連結 |
| `capture-pane -p -t <id> [-S -N]` | `capture` | `-S -N`→`lines: N`。`-p`でtextをstdoutへ |
| `select-pane -t <id>` | `ptygrid/focus` | |
| `kill-pane -t <id>` | `kill` | |
| `list-panes` | `list` | ID列挙 |
| `display-message -p '#{pane_id}'` | （`TMUX_PANE`優先、無ければ`get_self_id`） | |
| `list-sessions` / `has-session`等 | RPCなし | socket不要で成功を返す |
| 未知subcommand | RPCなし | **成功のno-op**（driving clientを落とさない）。「hookでteammate検知したのに`split-window`が来ない」ことを破損シグナルとして扱うのはapp側の責務 |

- RPC必須コマンドで`PTYGRID_TEAMS_SOCK`未設定・接続失敗・backend errorはstderr + exit 1。

## セマンティクス（重要）

- 本契約は**内部契約**であり、Queen MCPの`spawn_agent`（mterm.yml定義名allowlist）とは
  別チャネル。Queen tool semanticsは変更しない。teammate spawnの制約は
  (1) config opt-in（Phase 4.2で`agents[].teams.mode: host`）、(2) socket token、
  (3) binary allowlist（`spawn_agent`実装側でargv0を検証、拒否は`-32001`）の3段。
- client→serverのnotification（id無し）はv1では無視する。
- 検証: `cd src-tauri/teams-backend && cargo test`（unit 18 + socket統合 8 + shim E2E 4）、
