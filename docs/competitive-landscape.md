...
**すでに持っている強み(Phase 3.8)**: マルチペイン + mterm.yml + Queen 18 toolsに加え、
Git review/commit、任意worktree分離、logical resume、process-tree resource監視、競合安全な
Pins/Notes、永続Inbox/Replyまで一体化している。

| 優先 | 内容 | 競合が強い領域 |
|---|---|---|
| Phase 3実装済み | Git、worktree、pins/notes、inbox/reply、cancellable await | Superset / Parallel Code / Conductor / HiveTerm |
