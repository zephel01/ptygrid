...
実装状況: 方式B（observe: read-only transcript ペイン）は Phase 4.1 で実装済み。方式A（host:
実 PTY teammate ペイン）も **Phase 4.2 で実装済み**（既定オフの opt-in 実験機能）。tmux 互換シム・
per-lead Unix socket RPC・env/PATH 注入・実 PTY teammate ペイン・フォールバック検知（→ observe 降格）・
...
inside/phase3.md の流儀（各リリースは独立 release、既存契約を壊さない、CONTRACT 先行、両チェック通過）に従う。
