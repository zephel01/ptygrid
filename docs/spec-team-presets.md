...
> **偵察所見（2026-07-17）による改訂**: 当初案の「自力で詰まったと判断したら」という
> **難易度の自己判断はトリガーとして機能しない**。ローカルモデルは難問にも自信を
> 持って普通に答えてしまい、「詰まった」と自己申告しない（かつ、答えられた≠正しい）。
> トリガーは**客観条件**で書く。

> 一次担当（local）への指示例: 「次のどれかに当てはまったら、自分で答えられるか
> どうかに関係なく必ずエスカレーションする: ①テスト/ビルドが同じ原因で2回連続失敗
> ②公開API・保存データ形式・セキュリティ境界・並行処理設計に触る変更 ③人間に
> 『opusに聞いて』と言われた。手順: `spawn_agent` で `opus` を起動 → inbox で問題の
> 要約・試したこと・暫定案を送る → `await` で回答を待って作業に反映。②に該当する
> 変更は完了報告の前に opus のレビューを受ける。完了したら opus のペインは人間が閉じる。」

②のレビュー必須ルールは「自信を持って間違える」ケースの保険でもある（エスカレーション
を自己申告からワークフロー条件に変えることで、確実に発火する）。
...
> ⑤ **R2b（客観トリガー発火）は不合格（エージェント発・当該ローカルモデル）**:
> 「テスト変更禁止・9面上限変更禁止」の明示制約下で、local は 2回失敗→エスカレーション
> に進まず、**制約を破って src-tauri 3ファイルの上限定数を 9→12 に書き換えた**
> （TEAM_SESSION_CAP / teams_hooks・teams_host の GRID_MAX_PANES）。しかも drill テストは
> リテラル比較で自己完結のため、この変更では通らない。「自分が書き換えた後の値」を
> 根拠に「既に 12 になっている」と正当化する挙動も観測（自己編集の事後正当化）。
> 3ファイルは HEAD から復元済み。**エージェント発エスカレーションはこのモデルでは
> 既定にできない** — 7章の既定パターンは「人間発（条件③）+ 条件②レビューの人間側徹底」
> に倒すのが現時点の判定。モデルを替えての再試行は任意。
