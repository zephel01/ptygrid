...
- **ツールバー左**: 「+ Shell」ボタン(ペイン追加)、**作業フォルダ**の入力欄＋「読み込み」ボタン(例 `~/works/hoge`。先頭 `~` 可)、読み込み後は設定ファイルの由来バッジ(`設定: プロジェクト内 / 起動フォルダ / ~/.ptygrid`)と ptygrid.yml で定義したエージェントのチップ(クリックで起動)
- **ツールバー右**: 「cd…」バッジ(一括cd)、Gitパネルのボタン、全ペインのCPU/メモリ合計、「● Queen :39237」バッジ、ペイン数
  - 🟢 緑 = 稼働中 / 🔴 赤 = 停止 / ⚪ 灰 = 無効(`queen.enabled: false`)
  - クリックで Claude Code 用の登録コマンドをクリップボードにコピー

### 一括cd

ツールバー右の **「cd…」バッジ**を押すと、開いているペインへ `cd <dir>` + Enter を
まとめて送信するポップオーバーが開きます。ペインごとに同じ `cd` を打ち直す手間を省きます。

- **プロジェクトルート**: プロジェクトの置き場所（例: `~/works/project`）を一度設定すると
  アプリ設定として永続化されます。ポップオーバー上部の入力欄にパスを入れて「保存」を押すと、
  `~` をホーム展開したうえで**存在するディレクトリか検証**してから保存します（不正なら
  ポップオーバー内にエラー表示）。設定は `app-settings.json` に保存され、プロジェクトを
  切り替えても保持されます。
- **フォルダ一覧**: ルート設定済みなら、開いたときにルート直下の**非隠しフォルダ**が一覧
  表示されます（名前順・最大200件）。クリックすると選択されてハイライトし、下の入力欄に
  `<root>/<フォルダ名>` が入ります。毎回フルパスを手打ちするタイプミスを防げます。
- **ディレクトリ入力欄**: 移動先を入力します。`notemake` のような**フォルダ名だけ**を入れると
  プロジェクトルート基準で `<root>/notemake` に解決されます。`/...`（絶対パス）や
  `~/works/...`（先頭 `~`）はそのまま使われ、ルート未設定なら入力値をそのまま送ります。
  先頭 `~` はシェルでホーム展開されます。パスはシングルクォートで安全に囲んで送るため、
  空白や記号を含むパスもそのまま渡ります。入力値はアプリ実行中は保持されます。
- **対象(既定)**: 実行中の **シェルのペインだけ**です(`kind` が pty・状態が running・
  フォアグラウンドが sh/bash/zsh/fish 等)。フォアグラウンド名が取得できないペインは
  シェル扱いにします。transcript(読み取り専用)ペインは常に対象外です。
- **「CLI実行中のペインにも送る」**(既定オフ): オンにすると running な全 pty ペインが
  対象になります。実行中の Claude Code / Codex 等への**入力**になるため、注意文が表示されます。
- ポップオーバーには「Nペインに送信」と対象数が出ます。対象が0件なら送信ボタンは押せません。
  送信後は「Nペインに cd を送信しました」とトーストが出ます。Escape か外側クリックで閉じます。
...
| 複数ペインへ一括cd | ツールバーの「cd…」バッジ(既定は実行中シェルのペインのみ対象) |
...
~/.ptygrid`）を示すバッジが出ます（hover で実際のパスと作業フォルダを表示）。作業フォルダは
