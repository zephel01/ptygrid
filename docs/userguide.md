...
- **ssh 接続先の表示**: ペインのフォアグラウンドが `ssh` のとき、ペインヘッダーとステータスサイドバーの表示名が `ssh user@host` のように**接続先付き**になります。`.ssh/config` のエイリアス名、`-l user` 指定(`user@host` に畳み込み)、`-p` 等の値付きオプションのスキップに対応。ssh を抜けると次のサンプリング(1秒)で自動的に元へ戻ります。別ホストへの打ち間違い防止に
