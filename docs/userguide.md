...
- **ツールバー左**: 「+ Shell」ボタン(ペイン追加)、ptygrid.yml で定義したエージェントのチップ(クリックで起動)
...
プロジェクトルートに `ptygrid.yml` を置き、アプリのツールバーから読み込みます。
旧ファイル名 `mterm.yml` も互換のため読み込めます(両方ある場合は `ptygrid.yml` を優先)。
