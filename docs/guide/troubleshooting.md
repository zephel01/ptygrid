...
## ptygridのウィンドウが「勝手に落ちる」(2026-07-17の事例)

- **症状**: ptygrid自身のリポジトリを作業フォルダにしてエージェント検証をしていたら、
  ペインに長文を投入した前後でptygridのウィンドウごと数回消えた。
- **切り分け**: macOSのクラッシュレポート(`~/Library/Logs/DiagnosticReports`)に
  ptygridの`.ips`が1件も無い = 本物のクラッシュ(segfault/abort)ではない。
  一方で`src-tauri/target/debug/ptygrid`のmtimeが「落ちた」時刻直後に更新されていた。
- **原因**: `npm run tauri dev`の**ファイル監視**。`src-tauri/`配下の変更(テストファイルの
  作成、エージェントによるソース編集など)を検知すると、tauri devはアプリをkillして
  再ビルド→再起動する。ドッグフーディングでptygrid自身を作業対象にすると、
  エージェントがsrc-tauriを触るたびにウィンドウが落ちたように見える
  (数分のRustビルド後に自動で戻る)。ペースト自体は無関係(タイミングが重なるだけ)。
- **対処**: 検証セッションでは監視を切って起動する:

  ```bash
  npm run tauri -- dev --no-watch
  ```

  もしくはビルド済みアプリ(`npm run tauri build`の成果物)で検証する。
  ソース変更を反映したいときだけ手動で再起動する。
- **教訓**: 「アプリが落ちた」と思ったらまずDiagnosticReportsを見る。`.ips`が無い
  「クラッシュ」は、外部からのkill(dev watcher / OOM以外の管理プロセス)を疑う。
