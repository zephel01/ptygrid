...
| バージョン | 内容 |
|---|---|
| ~~v0.4.0 / v0.4.1~~ | 個別タグは打たず **v0.4.2 に集約**（下記参照） |
| **v0.4.2** | Phase 4.0（hooks 受信基盤）〜 4.1（observe）〜 4.2（host モード実験）+ UXトラック（作業フォルダ/設定分離・一括cd・片付けUX 等）一式 = **最初のタグ。現時点の main** |
| v0.4.3 | Phase 4.3（Queen team preset）または 4.2 の修正 |
| v0.5.0 | Phase 5 系の最初のリリース（スコープは 4.3 完了時に決定） |

> 注: v0.1.0 のまま Phase 4.2 まで進めたため、遡及タグ（v0.4.0/v0.4.1）は付けず、
> 最初のリリースタグを **v0.4.2** として現在の到達点に打つ。以降は 1 Phase = 1 patch。
