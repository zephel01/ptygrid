...
### Phase 4.3 — Queen team preset（方式C、Claude Code 内部に非依存）【実装済み・リリース待ち】

方針決定・**実装完了（2026-07-17）**。詳細仕様と実装時の設計差分は
[spec-team-presets.md](spec-team-presets.md)、wire 契約は CONTRACT.md「Phase 4.3 追加契約」。
...
残タスク: 実機偵察（coderouter 構成での Queen 接続・エスカレーション一連）と、
リリース手順（version bump + タグ）。
...
| **v0.4.6** | Phase 4.3（Queen team preset: `team_presets` 宣言 + `spawn_team`（19本目）+ 👥 一括起動 UI + example/team-preset）。cargo test 210 / svelte-check 0。**現時点の main** |
