...
### 8.1b 実装中に見つかり、その場で決めたこと（2026-08-04〜05）

- **コピーごとの mailbox（実装済み）**: 本書は 3.9 で「kickoff の宛先は agent 名の mailbox」と
  書いており、そのまま実装した。実機 1 回目（2026-08-05）で、`onEach` のコピーは全部が同じ
  agent 定義から起動するため **N 個のペインが 1 つの mailbox を奪い合う**ことが分かった
  （レビュアーのペインが「待機で 3 件届いたので、いちばん新しい id を選び」と報告した）。
  返信は thread root で相関されるので run の結果は正しいが、どのペインがどの unit を担当するかが
  到着タイミング次第になり、2 つのペインが同じ unit を取る余地も残る。**是正**: `onEach` の
  コピーの宛先を `wf/<run_id>/<step_id>` に分け、その名前を `PTYGRID_MAILBOX` 環境変数で
  ペインへ渡す（kickoff 本文では渡せない — エージェントは kickoff が届く前に await している
  必要があるため）。`onEach` 以外の step は従来どおり agent 定義名のまま。併せて V11
  （`onEach` step の id は 64 バイト以下）を追加した。**3.9 と 4.3 の記述はこの決定で上書きされる。**
- **4.3 のサンプルは `cmd: "claude"` のままである（未修正）**: 実際には kickoff は durable inbox に
  置かれるだけでペインには打ち込まれないため、`cmd` に「起動直後に await を呼べ」という
  プロンプトを埋めないとエージェントは起動画面のまま止まる。動く形は
  `example/review-as-you-go` を見ること。
