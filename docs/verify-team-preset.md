...
1. E2E-1 の続きで、local に**わざと難しい依頼**を投げる
   （例: 「session.rs の autorestart 世代管理の設計上の弱点を挙げて」等、
   ローカルモデルには荷が重いもの）
2. instructions の規約どおり local が自力判断で `spawn_agent("opus")` →
   inbox 依頼 → `await` まで進むかを観察（進まなければ「opus に聞いて」と一言だけ促す。
   それでもダメなら人間が ▶ で opus を起動し依頼文を送る = 人間発フォールバック）
3. **合格**:
   - どの経路であれ **opus の回答が local の作業に反映されて**タスクが完了する
   - opus が動いたのはこの難問区間だけ（router ログ + API 使用量で確認 = G3）
   - 完了後、opus のペインを閉じて日常編の体制に戻れる
4. **記録**: エージェント発で回ったか、人間発フォールバックだったか。
